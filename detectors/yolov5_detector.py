...
class Yolov5Detector(BaseDetector):
    ...
    def load_model(self):
        weights_path = "pretrained-models/yolov5/yolov5s.pt"
    ...
    def predict_and_save(self, image: ch.Tensor, path: str, target: int = None, untarget: int = None, is_targeted: bool = True, threshold: float = 0.7, format: str = "RGB", gt_bbox: List[int] = None, result_dict: bool = False) -> Any:
        ...
        with ch.no_grad():
            ...
            img_tensor = TF.to_tensor(image_pil).unsqueeze(0).to(dtype=ch.float32, device=next(self.model.parameters()).device)
        ...
        if dets is not None and len(dets) > 0:
            ...
            for *xyxy, conf, cls in dets:
                coords = xyxy[:4]
                if len(coords) == 1 and coords[0].dim() == 1 and coords[0].numel() == 4:
                    x1, y1, x2, y2 = map(float, coords[0])
                elif all(coord.ndim == 0 for coord in coords):
                    x1, y1, x2, y2 = (float(coord.item()) for coord in coords)
                elif all(coord.shape == coords[0].shape for coord in coords) and coords[0].numel() == 18900:
                    # Skip large tensors that are not valid bbox coordinates
                    continue
                else:
                    raise ValueError(f"Expected 4 scalar tensors or a single tensor of shape [4], got: {[coord.shape for coord in coords]}")
                x1, y1, x2, y2 = (float(coord.item()) for coord in coords)
                x1 *= scale_x
                x2 *= scale_x
                y1 *= scale_y
    ...
    def zero_grad(self):
        # YOLOv5 is not typically used with backward passes; placeholder.
    ...
    def resolve_label_index(self, label):
        ...
        if label not in label_lookup:
            raise ValueError(f"Label '{label}' not found in YOLOv5 COCO class list.")
