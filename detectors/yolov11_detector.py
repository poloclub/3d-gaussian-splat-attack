...
class Yolov11Detector(BaseDetector):
    ...
    def load_model(self):
        ...
        cfg_path = "pretrained-models/yolov11/yolov11.yaml"
        weights_path = "pretrained-models/yolov11/yolov11.pt"

        with open(cfg_path, "r") as f:
            model_cfg = yaml.safe_load(f)
            # Set the model scale to 'n' (nano) by default
            depth, width, max_channels = model_cfg['scales']['n']
            model_cfg['depth_multiple'] = depth
            model_cfg['width_multiple'] = width
            model_cfg['max_channels'] = max_channels
    ...
    def predict_and_save(self, image: ch.Tensor, path: str, target: int = None, untarget: int = None, is_targeted: bool = True, threshold: float = 0.7, format: str = "RGB", gt_bbox: List[int] = None, result_dict: bool = False, image_id: int = None) -> Any:
        ...
        with ch.no_grad():
            ...
            if not hasattr(self, "yolo_wrapper"):
                self.yolo_wrapper = YOLO("pretrained-models/yolov11/yolov11.pt", task="detect")
    ...
    def zero_grad(self):
        # YOLOv11 is not typically used with backward passes; placeholder.
    ...
    def resolve_label_index(self, label):
        ...
        if label not in label_lookup:
            raise ValueError(f"Label '{label}' not found in YOLOv11 COCO class list.")
