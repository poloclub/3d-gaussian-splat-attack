...
def annotate_classes(img, classes):
    """
    Annotate the image with class indices at their first encountered positions.

    Args:
    - img (PIL.Image): The input image to annotate.
    - classes (np.array): A NumPy array of unsigned integers representing class indices.
    ...
    font = ImageFont.load_default(font_path,12)  # Load default font
    color = (255, 255, 255)  # RGB color for text

    # Create a set to keep track of annotated classes
    annotated_classes = set()
    len_unique = np.unique(classes.cpu().numpy()).shape[0]

    # Get the height and width of the classes tensor
    H, W = classes.shape

    # Iterate through each pixel of the classes tensor
    for i in range(H):
        for j in range(W):
            class_index = classes[i, j].item()
            if len(list(set(annotated_classes))) == len_unique:
                break
            if class_index not in annotated_classes:
                # Annotate the image with the class index
                x, y = (j, i)  # Use the pixel position for annotation
                draw.text((x, y), str(class_index), color, font=font)
                # Add the class index to the set of annotated classes
                annotated_classes.add(class_index)
