...
class GaussianModel:
    ...
    def concat_setup(self, feature_name, tensor_to_concat, requires_grad):
        """
        Concatenates a given tensor with an existing feature and re-establishes it as an nn.Parameter.

        Args:
            feature_name (str): The name of the feature to concatenate with.
            tensor_to_concat (torch.Tensor): The tensor to concatenate.
            requires_grad (bool): Whether the resulting tensor requires gradients.
        """
        # Get the existing feature tensor
        existing_feature = getattr(self, f"_{feature_name}")

        # Concatenate the tensors along the first dimension
        concatenated_tensor = torch.cat((existing_feature, tensor_to_concat), dim=0)

        # Re-establish the concatenated tensor as an nn.Parameter with the specified gradient setting
        concatenated_parameter = nn.Parameter(concatenated_tensor.detach().clone().requires_grad_(requires_grad))

        # Set the new parameter back to the object
        setattr(self, f"_{feature_name}", concatenated_parameter)
    ...
    def load_ply(self, path):
        ...
        try:
            ...
            print("No Gaussian groups find in .ply file")
